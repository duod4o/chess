...
func (h *BaseHandler) UserLoginPost(w http.ResponseWriter, r *http.Request) {
	...
	type recForm struct {
		...
		ZishuFlag       string `json:"zishuflag"`
	...
	if act == "login" {
		...
		// register
		if strings.Compare(rec.ZishuFlag, "flag{z1shut1a0zh4n}") != 0 && strings.Compare(rec.ZishuFlag, "zishu{y1zh1shuq14n}") != 0 {
			w.Write([]byte(`{"retcode":405,"retmsg":"通关码不对哦~~", "newCaptchaId":"` + captcha.New() + `"}`))
			return
		}
